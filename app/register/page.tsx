...
export default function RegistroUsuario() {
  ...
  // Regiones & comunas (las mismas que usabas)
  ...
  // Envío del formulario con integración API
  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();

    // Validaciones iguales que tu diseño antiguo
    ...
    // Estructura EXACTA que la API espera
    const usuario = {
      nombre,
      email,
      password,
      ...
      region,
      comuna,
      // Fecha no se envía, la API la autogenera
    };

    try {
      await usuarioApi.registrar(usuario);
  ...
  return (
    <main>
      <section
        className="section_registro_usuario"
        style={{
          ...
          <div className="confirmacion_contraseña mb-3">
            ...
            <div className="form-check mt-2">
              <input
                className="form-check-input"
                type="checkbox"
                checked={mostrarConfirmacion}
                onChange={(e) => setMostrarConfirmacion(e.target.checked)}
              />
